...
class WriteArticleForm(FlaskForm):
    ...
    url = StringField('URL', validators=[DataRequired(),
                                         AlphaDash(message="De url mag alleen maar tekst en streepjes bevatten")], default="URL...")
    ...
    def validate_summary(self, summary):
        article = Article.query.filter_by(url=summary.data).first()
        if article is not None:
            raise ValidationError("Er bestaat al een artikel met deze samenvatting, kies een andere samenvatting")
